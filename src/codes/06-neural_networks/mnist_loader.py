...
def load_MNIST(path):
	...
	validation_results = [unit_vector(y) for y in validation_data[1]]
	validation_data = list(zip(validation_inputs, validation_results))

	# testing data
	testing_inputs = [np.reshape(x, (784, )) for x in testing_data[0]]
	testing_results = [unit_vector(y) for y in testing_data[1]]
